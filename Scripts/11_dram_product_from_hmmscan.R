...
dbcan_mqhq <- read_tsv("Clean_Data/GH_in_MQHQ_bins.tsv") 
...
#non MQHQ bins snuck into the DRAM run so removing here
dram_product <- dram_product %>%
  rownames_to_column(var = "bins") %>%
  filter(bins != "KS_CoAss_bin.67")

write_tsv(dram_product, "Clean_Data/dram_product_hmmscan.tsv")